...
+ (BOOL)appendHyphenForCardnumber:(UITextField*)textField
...
+ (BOOL)appendHyphenForMobilenumber:(UITextField*)textField
                  replacementString:(NSString*)string
      shouldChangeCharactersInRange:(NSRange)range {
  // Reject appending non-digit characters
  if (range.length == 0 &&
...
+ (BOOL)validateCVVNumber:(UITextField*)textField
                replacementString:(NSString*)string
    shouldChangeCharactersInRange:(NSRange)range {
  // CVV validation
  ...
  for (int i = 0; i < [string length]; i++) {
    ...
    if ([myCharSet characterIsMember:c]) {
      ...
      } else if ([scheme caseInsensitiveCompare:@"amex"] != NSOrderedSame) {
...
+ (BOOL)validateCVVNumber:(UITextField*)textField
  ...
  // if amex allow 4 digits, if non amex only 3 should allowed.
  NSString* scheme =
      [CTSUtility fetchCardSchemeForCardNumber:
                      [cardNumber stringByReplacingOccurrencesOfString:@"-"
                                                            withString:@""]];
  NSInteger textfieldLength =
      textField.text.length - range.length + string.length;
  NSCharacterSet* myCharSet =
